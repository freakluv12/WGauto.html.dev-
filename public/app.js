...
async function showReceiveInventoryModal() {
    ...
    try {
        ...
        allProducts.forEach(p => {
            optionsHTML += `<option value="${p.id}" data-last-price="${p.last_purchase_price || ''}" data-currency="${p.last_currency || 'USD'}">${p.category_name} > ${p.subcategory_name} > ${p.name}</option>`;
        });
        
        document.getElementById('receiveProductId').innerHTML = optionsHTML;
    } catch (error) {
        console.error('Error loading products:', error);
    }
}

// НОВАЯ ФУНКЦИЯ: Автопостановка последней цены при выборе товара
function onReceiveProductChange() {
    const select = document.getElementById('receiveProductId');
    const selectedOption = select.options[select.selectedIndex];
    
    if (selectedOption && selectedOption.value) {
        const lastPrice = selectedOption.getAttribute('data-last-price');
        const lastCurrency = selectedOption.getAttribute('data-currency');
        
        if (lastPrice) {
            document.getElementById('receivePurchasePrice').value = lastPrice;
            document.getElementById('receiveCurrency').value = lastCurrency;
        }
    }
}

// НОВАЯ ФУНКЦИЯ: Оприходование товара
async function receiveInventory() {
    const data = {
        product_id: parseInt(document.getElementById('receiveProductId').value),
    ...
    if (!data.product_id || !data.quantity || data.quantity <= 0) {
        alert('Выберите товар и укажите положительное количество');
        return;
    }
    ...
    try {
        ...
        if (response && response.ok) {
            ...
            // Очистить форму
            document.getElementById('receiveProductId').value = '';
            ...
            const error = await response.json();
            alert('Ошибка оприходования: ' + (error.error || 'Unknown'));
        }
    } catch (error) {
