...
app.get('/api/stats/dashboard', authenticateToken, async (req, res) => {
  try {
    ...
    const incomeQuery = `SELECT currency, SUM(amount) as total FROM transactions WHERE type = 'income' ${userFilter} GROUP BY currency`;
    const income = await pool.query(incomeQuery, params);

    const expenseQuery = `SELECT currency, SUM(amount) as total FROM transactions WHERE type = 'expense' ${userFilter} GROUP BY currency`;
    const expenses = await pool.query(expenseQuery, params);

    const carsQuery = `SELECT status, COUNT(*) as count FROM cars WHERE 1=1 ${userFilter} GROUP BY status`;
    const cars = await pool.query(carsQuery, params);

    const activeRentalsQuery = `SELECT COUNT(*) as count FROM rentals WHERE status = 'active' ${userFilter}`;
...
app.post('/api/rentals/:id/complete', authenticateToken, async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [rentalData.car_id, req.user.id, 'income', rentalData.total_amount, rentalData.currency, 'rental', `Rental income from ${rentalData.client_name}`, rentalId]
    );
...
app.get('/api/warehouse/subcategories/:categoryId', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT * FROM subcategories WHERE category_id = $1 ORDER BY name', [categoryId]);
    res.json(result.rows);
  } catch (error) {
...
app.put('/api/warehouse/products/:id/prices', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [purchase_price, sale_price, currency, productId]
    );
...
app.post('/api/warehouse/inventory/receive', authenticateToken, async (req, res) => {
  try {
    ...
    if (sale_price || purchase_price) {
      await pool.query(
        'UPDATE products SET purchase_price = COALESCE($1, purchase_price), sale_price = COALESCE($2, sale_price), currency = COALESCE($3, currency) WHERE id = $4',
        [purchase_price, sale_price, currency, product_id]
      );
    }
...
app.post('/api/warehouse/sales', authenticateToken, async (req, res) => {
  try {
    ...
    try {
      ...
      // Get oldest inventory
      const inventoryCheck = await client.query(
        'SELECT id, quantity FROM inventory WHERE product_id = $1 AND quantity > 0 ORDER BY received_date LIMIT 1',
        [product_id]
      );
      ...
      const saleResult = await client.query(
        ...
        [inv.id, product_id, quantity, sale_price, cost_price || null, currency, buyer_name || '', buyer_phone || '', notes || '', req.user.id]
      );
